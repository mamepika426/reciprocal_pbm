...
class Raw_data:
    ...
    def load_data_in_libsvm_format(
        ...
        # ここから山口用
        train_set_dict = gen_data.update_train_set_yamaguchi()
        self.feature_size = train_set_dict['feature_size']
        self.rank_list_size = train_set_dict['rank_list_size']
        self.features = train_set_dict['features']
        self.qids = train_set_dict['qids']
        self.initial_list = train_set_dict['initial_list']
        self.labels = train_set_dict['labels']

        print("feature_size: ", self.feature_size)
        print("qids: ", len(self.qids))
        print("initial_list: ", len(self.initial_list))
        print("labels: ", len(self.labels))
        print("features", len(self.features), len(self.features[0]))
        # print("features_dict.keys", self.features_dict.values())
        return
