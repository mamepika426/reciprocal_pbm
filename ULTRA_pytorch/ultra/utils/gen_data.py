...
def gen_logdata(sender_profiles: np.ndarray, receiver_profiles: np.ndarray, preferences: np.ndarray
    ...
    for n in range(num_search):
        ...
        labels.append(list(sheet['gamma_sender']))
        for receiver in receivers_list[n]:
            features.append(list(sender_profiles[senders_list[n]]) + list(receiver_profiles[receiver]))
    
    return labels, features


def update_train_set_yamaguchi() -> dict:
    ...
    labels, features = gen_logdata(male_profiles, female_profiles, male_preferences, S, NUM_SHEETS, rel_male2female, rel_female2male)
    ...
    train_set_dict = {'feature_size': 200,
                      'rank_list_size': S,
                      'features': features,
                      'qids': [str(n+1) for n in range(NUM_SHEETS)],
                      'initial_list': [[10*n + k for k in range(10)] for n in range(NUM_SHEETS)],
                      'labels': labels}
    print("山口検証用データ作成完了")
    return train_set_dict
